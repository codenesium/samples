...
namespace TwitterNS.Api.Web
{
	public abstract class AbstractLocationController : AbstractApiController
	{
		...
		public async virtual Task<IActionResult> Tweets(int locationId, int? limit, int? offset)
		{
			SearchQuery query = new SearchQuery();
			if (!query.Process(this.MaxLimit, this.DefaultLimit, limit, offset, this.ControllerContext.HttpContext.Request.Query.ToDictionary(q => q.Key, q => q.Value)))
			{
			...
			List<ApiTweetResponseModel> response = await this.LocationService.Tweets(locationId, query.Limit, query.Offset);
		...
		public async virtual Task<IActionResult> Users(int locationLocationId, int? limit, int? offset)
		{
			SearchQuery query = new SearchQuery();
			if (!query.Process(this.MaxLimit, this.DefaultLimit, limit, offset, this.ControllerContext.HttpContext.Request.Query.ToDictionary(q => q.Key, q => q.Value)))
			{
			...
			List<ApiUserResponseModel> response = await this.LocationService.Users(locationLocationId, query.Limit, query.Offset);
...
/*<Codenesium>
    <Hash>389688cb9fa05033ddc1fd74b200a59a</Hash>
</Codenesium>*/