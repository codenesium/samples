...
interface Props {
   ...
   const QuoteTweetCreateDisplay: React.SFC<FormikProps<QuoteTweetViewModel>> = (props: FormikProps<QuoteTweetViewModel>) => {
   ...
   {
        let response = '';
        if(props.touched[name as keyof QuoteTweetViewModel]  && props.errors[name as keyof QuoteTweetViewModel]) {
   ...
   return (<form onSubmit={props.handleSubmit} role="form">            
						...
						<div className="form-group row">
                        <label htmlFor="name" className={errorExistForField("retweeterUserId") ? ("col-sm-2 col-form-label is-invalid") : "col-sm-2 col-form-label"}>Retweeter_user_id</label>
					    <div className="col-sm-12">
                             <Field type="textbox" name="retweeterUserId" className={errorExistForField("retweeterUserId") ? "form-control is-invalid" : "form-control"} />
                            {errorExistForField("retweeterUserId") && <small className="text-danger">{errorsForField("retweeterUserId")}</small>}
                        </div>
                    </div>

						<div className="form-group row">
                        <label htmlFor="name" className={errorExistForField("sourceTweetId") ? ("col-sm-2 col-form-label is-invalid") : "col-sm-2 col-form-label"}>Source_tweet_id</label>
					    <div className="col-sm-12">
                             <Field type="textbox" name="sourceTweetId" className={errorExistForField("sourceTweetId") ? "form-control is-invalid" : "form-control"} />
                            {errorExistForField("sourceTweetId") && <small className="text-danger">{errorsForField("sourceTweetId")}</small>}
                        </div>
                    </div>
...
const QuoteTweetCreate = withFormik<Props, QuoteTweetViewModel>({
    mapPropsToValues: props => {
		...
		{
			response.setProperties(props.model!.content,props.model!.date,props.model!.quoteTweetId,props.model!.retweeterUserId,props.model!.sourceTweetId,props.model!.time);	
		}
		return response;
      },
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.post(Constants.ApiEndpoint + ApiRoutes.QuoteTweets,
        ...
        .then(resp => {
            let response = resp.data as CreateResponse<Api.QuoteTweetClientRequestModel>;
  ...
  export default class QuoteTweetCreateComponent extends React.Component<QuoteTweetCreateComponentProps, QuoteTweetCreateComponentState> {
...
/*<Codenesium>
    <Hash>bd79caac181b72913b2f61ff47ff4064</Hash>
</Codenesium>*/