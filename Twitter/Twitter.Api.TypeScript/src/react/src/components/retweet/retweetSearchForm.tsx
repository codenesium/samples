...
export default class RetweetSearchComponent extends React.Component<RetweetSearchComponentProps, RetweetSearchComponentState> {

    state = ({deleteSubmitted:false, deleteSuccess:false, deleteResponse:'', records:new Array<RetweetViewModel>(), filteredRecords:new Array<RetweetViewModel>(), searchValue:'', loading:false, loaded:true, errorOccurred:false, errorMessage:''});
    
    componentDidMount () {
        this.loadRecords();
    }

    handleEditClick(e:any, row:Api.RetweetClientResponseModel) {
         this.props.history.push(ClientRoutes.Retweets + '/edit/' + row.id);
    }

    handleDetailClick(e:any, row:Api.RetweetClientResponseModel) {
    ...
    handleDeleteClick(e:any, row:Api.RetweetClientResponseModel) {
        axios.delete(Constants.ApiEndpoint + ApiRoutes.Retweets + '/' + row.id,
        {
            headers: {
   ...
   loadRecords(query:string = '') {
	   ...
	   let searchEndpoint = Constants.ApiEndpoint + ApiRoutes.Retweets + '?limit=100';
	   ...
	   .then(resp => {
		   ...
		   console.log(error);
		   this.setState({records:new Array<RetweetViewModel>(),filteredRecords:new Array<RetweetViewModel>(), loading:false, loaded:false, errorOccurred:true, errorMessage:'Error from API'});
	   })
    }
        ...
        if(this.state.loading) {
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.RetweetClientResponseModel)}} ><i className="fas fa-search"></i></button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.RetweetClientResponseModel)}} ><i className="fas fa-edit"></i></button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.RetweetClientResponseModel)}} ><i className="far fa-trash-alt"></i></button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>cddb5ae702535f2c68ea37b989a6e9e0</Hash>
</Codenesium>*/