...
export default class TweetSearchComponent extends React.Component<
  TweetSearchComponentProps,
  TweetSearchComponentState
> {
  state = {
    deleteSubmitted: false,
    deleteSuccess: false,
    deleteResponse: '',
    records: new Array<TweetViewModel>(),
    filteredRecords: new Array<TweetViewModel>(),
    searchValue: '',
    loading: false,
    loaded: true,
    errorOccurred: false,
  ...
  handleDeleteClick(e: any, row: Api.TweetClientResponseModel) {
    axios
      .delete(Constants.ApiEndpoint + ApiRoutes.Tweets + '/' + row.tweetId, {
  ...
  loadRecords(query: string = '') {
    ...
    let searchEndpoint =
      Constants.ApiEndpoint + ApiRoutes.Tweets + '?limit=100';
    ...
    axios
      .get<Array<Api.TweetClientResponseModel>>(searchEndpoint, {
        headers: GlobalUtilities.defaultHeaders(),
      })
      .then(response => {
...
export const WrappedTweetSearchComponent = Form.create({
  name: 'Tweet Search',
})(TweetSearchComponent);


/*<Codenesium>
    <Hash>4e2e7bed2309173292482a265d989222</Hash>
    <Hello>
