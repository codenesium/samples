...
interface Props {
   ...
   const TweetCreateDisplay: React.SFC<FormikProps<TweetViewModel>> = (props: FormikProps<TweetViewModel>) => {
   ...
   {
        let response = '';
        if(props.touched[name as keyof TweetViewModel]  && props.errors[name as keyof TweetViewModel]) {
   ...
   return (<form onSubmit={props.handleSubmit} role="form">            
						...
						<div className="form-group row">
                        <label htmlFor="name" className={errorExistForField("locationId") ? ("col-sm-2 col-form-label is-invalid") : "col-sm-2 col-form-label"}>Location_id</label>
					    <div className="col-sm-12">
                             <Field type="textbox" name="locationId" className={errorExistForField("locationId") ? "form-control is-invalid" : "form-control"} />
                            {errorExistForField("locationId") && <small className="text-danger">{errorsForField("locationId")}</small>}
                        </div>
                    </div>
						...
						<div className="form-group row">
                        <label htmlFor="name" className={errorExistForField("userUserId") ? ("col-sm-2 col-form-label is-invalid") : "col-sm-2 col-form-label"}>User_user_id</label>
					    <div className="col-sm-12">
                             <Field type="textbox" name="userUserId" className={errorExistForField("userUserId") ? "form-control is-invalid" : "form-control"} />
                            {errorExistForField("userUserId") && <small className="text-danger">{errorsForField("userUserId")}</small>}
                        </div>
                    </div>
...
const TweetCreate = withFormik<Props, TweetViewModel>({
    mapPropsToValues: props => {
		...
		{
			response.setProperties(props.model!.content,props.model!.date,props.model!.locationId,props.model!.time,props.model!.tweetId,props.model!.userUserId);	
		}
		return response;
      },
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.post(Constants.ApiEndpoint + ApiRoutes.Tweets,
        ...
        .then(resp => {
            let response = resp.data as CreateResponse<Api.TweetClientRequestModel>;
  ...
  export default class TweetCreateComponent extends React.Component<TweetCreateComponentProps, TweetCreateComponentState> {
...
/*<Codenesium>
    <Hash>b4e191ecc225cd9a46ba9850cdb62443</Hash>
</Codenesium>*/