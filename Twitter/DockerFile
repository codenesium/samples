...
WORKDIR Twitter.Api.TypeScript/src/react
...
RUN dotnet publish Twitter.Api.Web/Twitter.Api.Web.csproj -c Release  -o /publish -r alpine-x64 --self-contained
...
COPY --from=jsbuild Twitter.Api.TypeScript/src/react/build .

WORKDIR /publish
ENTRYPOINT ["./Twitter.Api.Web"]