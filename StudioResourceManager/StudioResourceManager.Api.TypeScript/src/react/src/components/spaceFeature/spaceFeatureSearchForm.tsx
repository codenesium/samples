...
export default class SpaceFeatureSearchComponent extends React.Component<SpaceFeatureSearchComponentProps, SpaceFeatureSearchComponentState> {
    ...
    state = ({deleteSubmitted:false, deleteSuccess:false, deleteResponse:'', records:new Array<SpaceFeatureViewModel>(), filteredRecords:new Array<SpaceFeatureViewModel>(), searchValue:'', loading:false, loaded:true, errorOccurred:false, errorMessage:''});
    ...
    handleEditClick(e:any, row:Api.SpaceFeatureClientResponseModel) {
         this.props.history.push(ClientRoutes.SpaceFeatures + '/edit/' + row.id);
    }

    handleDetailClick(e:any, row:Api.SpaceFeatureClientResponseModel) {
         this.props.history.push(ClientRoutes.SpaceFeatures + '/' + row.id);
    }

    handleCreateClick(e:any) {
        this.props.history.push(ClientRoutes.SpaceFeatures + '/create');
    }

    handleDeleteClick(e:any, row:Api.SpaceFeatureClientResponseModel) {
        axios.delete(Constants.ApiEndpoint + ApiRoutes.SpaceFeatures + '/' + row.id,
        {
            headers: {
   ...
   loadRecords(query:string = '') {
	   ...
	   let searchEndpoint = Constants.ApiEndpoint + ApiRoutes.SpaceFeatures + '?limit=100';
        ...
        if(this.state.loading) {
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.SpaceFeatureClientResponseModel)}} ><i className="fas fa-search"></i></button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.SpaceFeatureClientResponseModel)}} ><i className="fas fa-edit"></i></button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.SpaceFeatureClientResponseModel)}} ><i className="far fa-trash-alt"></i></button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>14d8cacb50ee8b95d2e3b263e6ae985e</Hash>
</Codenesium>*/