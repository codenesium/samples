...
WORKDIR StudioResourceManager.Api.TypeScript/src/react
...
RUN dotnet publish StudioResourceManager.Api.Web/StudioResourceManager.Api.Web.csproj -c Release  -o /publish -r alpine-x64 --self-contained
...
COPY --from=jsbuild StudioResourceManager.Api.TypeScript/src/react/build .

WORKDIR /publish
ENTRYPOINT ["./StudioResourceManager.Api.Web"]