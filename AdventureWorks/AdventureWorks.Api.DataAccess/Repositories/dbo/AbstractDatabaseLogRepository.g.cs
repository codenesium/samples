...
namespace AdventureWorksNS.Api.DataAccess
{
	public abstract class AbstractDatabaseLogRepository : AbstractRepository
	{
		...
		public virtual Task<List<DatabaseLog>> All(int limit = int.MaxValue, int offset = 0, string query = "")
		{
			...
			else
			{
				return this.Where(x =>
				                  ...
				                  x.ReservedEvent.StartsWith(query) ||
				                  x.ReservedObject.StartsWith(query) ||
			...
			Expression<Func<DatabaseLog, dynamic>> orderBy = null)
		{
			...
			return await this.Context.Set<DatabaseLog>()

			       .Where(predicate).AsQueryable().OrderBy(orderBy).Skip(offset).Take(limit).ToListAsync<DatabaseLog>();
		}
...
/*<Codenesium>
    <Hash>b2ec62b00c6586b04539baa1aa550b4a</Hash>
</Codenesium>*/