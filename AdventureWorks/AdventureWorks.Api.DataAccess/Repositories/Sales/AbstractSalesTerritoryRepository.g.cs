	...
	public abstract class AbstractSalesTerritoryRepository : AbstractRepository
	{
		...
		public virtual Task<List<SalesTerritory>> All(int limit = int.MaxValue, int offset = 0, string query = "")
		{
			...
			else
			{
				return this.Where(x =>
				                  ...
				                  x.ReservedGroup.StartsWith(query) ||
			...
			Expression<Func<SalesTerritory, dynamic>> orderBy = null)
		{
			...
			return await this.Context.Set<SalesTerritory>()
			       ...
			       .Where(predicate).AsQueryable().OrderBy(orderBy).Skip(offset).Take(limit).ToListAsync<SalesTerritory>();
		}
...
/*<Codenesium>
    <Hash>5e91524ffe17b7e61b90f5ac3dbfbba7</Hash>
</Codenesium>*/