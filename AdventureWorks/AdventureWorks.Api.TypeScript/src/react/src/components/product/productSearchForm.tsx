...
export default class ProductSearchComponent extends React.Component<ProductSearchComponentProps, ProductSearchComponentState> {

    state = ({deleteSubmitted:false, deleteSuccess:false, deleteResponse:'', records:new Array<ProductViewModel>(), filteredRecords:new Array<ProductViewModel>(), searchValue:'', loading:false, loaded:true, errorOccurred:false, errorMessage:''});
    ...
    handleDeleteClick(e:any, row:Api.ProductClientResponseModel) {
        axios.delete(Constants.ApiEndpoint + ApiRoutes.Products + '/' + row.productID,
        {
            headers: {
   ...
   loadRecords(query:string = '') {
	   ...
	   let searchEndpoint = Constants.ApiEndpoint + ApiRoutes.Products + '?limit=100';
	   ...
	   .then(resp => {
		   ...
		   console.log(error);
		   this.setState({records:new Array<ProductViewModel>(),filteredRecords:new Array<ProductViewModel>(), loading:false, loaded:false, errorOccurred:true, errorMessage:'Error from API'});
	   })
    }
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    columns: [
                      ...
                      Cell: (props) => {
                      return <span>{String(props.original.sizeUnitMeasureCode)}</span>;
                      }           
                    },  {
                      Header: 'StandardCost',
                      ...
                      Cell: (props) => {
                      return <span>{String(props.original.weightUnitMeasureCode)}</span>;
                      }           
                    },
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.ProductClientResponseModel)}} ><i className="fas fa-search"></i></button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.ProductClientResponseModel)}} ><i className="fas fa-edit"></i></button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.ProductClientResponseModel)}} ><i className="far fa-trash-alt"></i></button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>91d5a5842ff1987d5469ed6715f4b0c4</Hash>
</Codenesium>*/