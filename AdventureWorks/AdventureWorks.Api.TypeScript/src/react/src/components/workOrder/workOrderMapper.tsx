...
import WorkOrderViewModel from  './workOrderViewModel';
	...
	mapApiResponseToViewModel(dto: Api.WorkOrderClientResponseModel) : WorkOrderViewModel 
	{
		let response = new WorkOrderViewModel();
		response.setProperties(dto.dueDate,dto.endDate,dto.modifiedDate,dto.orderQty,dto.productID,dto.scrappedQty,dto.scrapReasonID,dto.startDate,dto.stockedQty,dto.workOrderID);
							...
							if(dto.scrapReasonIDNavigation != null)
				{
				  ...
				  dto.scrapReasonIDNavigation.modifiedDate,dto.scrapReasonIDNavigation.name,dto.scrapReasonIDNavigation.scrapReasonID
				  );
				}
	...
	mapViewModelToApiRequest(model: WorkOrderViewModel) : Api.WorkOrderClientRequestModel
	{
		let response = new Api.WorkOrderClientRequestModel();
		response.setProperties(model.dueDate,model.endDate,model.modifiedDate,model.orderQty,model.productID,model.scrappedQty,model.scrapReasonID,model.startDate,model.stockedQty,model.workOrderID);
		return response;
	}
};

/*<Codenesium>
    <Hash>5a50ceb149847c820bb37aeab6e61060</Hash>
</Codenesium>*/