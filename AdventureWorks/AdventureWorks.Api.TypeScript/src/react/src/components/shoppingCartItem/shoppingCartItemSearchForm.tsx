...
export default class ShoppingCartItemSearchComponent extends React.Component<ShoppingCartItemSearchComponentProps, ShoppingCartItemSearchComponentState> {

    state = ({deleteSubmitted:false, deleteSuccess:false, deleteResponse:'', records:new Array<ShoppingCartItemViewModel>(), filteredRecords:new Array<ShoppingCartItemViewModel>(), searchValue:'', loading:false, loaded:true, errorOccurred:false, errorMessage:''});
    ...
    handleEditClick(e:any, row:Api.ShoppingCartItemClientResponseModel) {
         this.props.history.push(ClientRoutes.ShoppingCartItems + '/edit/' + row.shoppingCartItemID);
    }

    handleDetailClick(e:any, row:Api.ShoppingCartItemClientResponseModel) {
         this.props.history.push(ClientRoutes.ShoppingCartItems + '/' + row.shoppingCartItemID);
    }
    ...
    handleDeleteClick(e:any, row:Api.ShoppingCartItemClientResponseModel) {
        axios.delete(Constants.ApiEndpoint + ApiRoutes.ShoppingCartItems + '/' + row.shoppingCartItemID,
        {
            headers: {
   ...
   loadRecords(query:string = '') {
	   ...
	   let searchEndpoint = Constants.ApiEndpoint + ApiRoutes.ShoppingCartItems + '?limit=100';
	   ...
	   .then(resp => {
		   ...
		   console.log(error);
		   this.setState({records:new Array<ShoppingCartItemViewModel>(),filteredRecords:new Array<ShoppingCartItemViewModel>(), loading:false, loaded:false, errorOccurred:true, errorMessage:'Error from API'});
	   })
    }
        ...
        if(this.state.loading) {
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.ShoppingCartItemClientResponseModel)}} ><i className="fas fa-search"></i></button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.ShoppingCartItemClientResponseModel)}} ><i className="fas fa-edit"></i></button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.ShoppingCartItemClientResponseModel)}} ><i className="far fa-trash-alt"></i></button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>acf2a089f60c3412fede0740e3239251</Hash>
</Codenesium>*/