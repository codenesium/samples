...
interface Props {
  ...
  const CurrencyEditDisplay = (props: FormikProps<CurrencyViewModel>) => {
   ...
   { 
        let response = '';
        if(props.touched[name as keyof CurrencyViewModel]  && props.errors[name as keyof CurrencyViewModel]) {
   ...
   return (
          ...
          <form onSubmit={props.handleSubmit} role="form">
                        ...
                        <label htmlFor="name" className={errorExistForField("currencyCode") ? ("col-sm-2 col-form-label is-invalid") : "col-sm-2 col-form-label"}>CurrencyCode</label>
					    <div className="col-sm-12">
                             <Field type="datetime-local" name="currencyCode" className={errorExistForField("currencyCode") ? "form-control is-invalid" : "form-control"} />
                            {errorExistForField("currencyCode") && <small className="text-danger">{errorsForField("currencyCode")}</small>}
                        </div>
                    </div>
							<div className="form-group row">
...
const CurrencyEdit = withFormik<Props, CurrencyViewModel>({
    mapPropsToValues: props => {
        let response = new CurrencyViewModel();
		response.setProperties(props.model!.currencyCode,props.model!.modifiedDate,props.model!.name);	
		return response;
      },
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.put(Constants.ApiEndpoint + ApiRoutes.Currencies +'/' + values.currencyCode,
        ...
        .then(resp => {
            let response = resp.data as UpdateResponse<Api.CurrencyClientRequestModel>;
  ...
  export default class CurrencyEditComponent extends React.Component<CurrencyEditComponentProps, CurrencyEditComponentState> {
    ...
    componentDidMount () {
        ...
        axios.get(Constants.ApiEndpoint + ApiRoutes.Currencies + '/' + this.props.match.params.currencyCode, {
            headers: {
                'Content-Type': 'application/json',
            }
        })
    ...
    render () {
        ...
        if (this.state.loading) {
            ...
            return (<CurrencyEdit model={this.state.model} />);
        } 
		else {
		  return null;
		}
...
/*<Codenesium>
    <Hash>dba3a94e07fe04872f75a1f05dea1453</Hash>
</Codenesium>*/