...
interface Props {
   ...
   const PhoneNumberTypeCreateDisplay: React.SFC<FormikProps<PhoneNumberTypeViewModel>> = (props: FormikProps<PhoneNumberTypeViewModel>) => {

   let status = props.status as CreateResponse<Api.PhoneNumberTypeClientRequestModel>;
   
   let errorsForField = (name:string) : string =>
   {
        let response = '';
        if(props.touched[name as keyof PhoneNumberTypeViewModel]  && props.errors[name as keyof PhoneNumberTypeViewModel]) {
...
const PhoneNumberTypeCreate = withFormik<Props, PhoneNumberTypeViewModel>({
    mapPropsToValues: props => {
		...
		{
			response.setProperties(props.model!.modifiedDate,props.model!.name,props.model!.phoneNumberTypeID);	
		}
		return response;
      },
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.post(Constants.ApiEndpoint + ApiRoutes.PhoneNumberTypes,
        ...
        .then(resp => {
            let response = resp.data as CreateResponse<Api.PhoneNumberTypeClientRequestModel>;
  ...
  export default class PhoneNumberTypeCreateComponent extends React.Component<PhoneNumberTypeCreateComponentProps, PhoneNumberTypeCreateComponentState> {
...
/*<Codenesium>
    <Hash>de403de7143ade3c0e4dc503db4b1391</Hash>
</Codenesium>*/