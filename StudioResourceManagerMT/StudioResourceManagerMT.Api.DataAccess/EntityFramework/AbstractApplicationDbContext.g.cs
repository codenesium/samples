...
namespace StudioResourceManagerMTNS.Api.DataAccess
{
	public abstract class AbstractApplicationDbContext : IdentityDbContext<AuthUser>
	{
		...
		protected override void OnModelCreating(ModelBuilder modelBuilder)
		{
			...
			modelBuilder.Entity<Rate>().HasQueryFilter(x => x.TenantId == this.TenantId && !x.IsDeleted);
			modelBuilder.Entity<Space>().HasQueryFilter(x => x.TenantId == this.TenantId && !x.IsDeleted);
			modelBuilder.Entity<SpaceFeature>().HasQueryFilter(x => x.TenantId == this.TenantId && !x.IsDeleted);
			modelBuilder.Entity<SpaceSpaceFeature>().HasQueryFilter(x => x.TenantId == this.TenantId && !x.IsDeleted);
			modelBuilder.Entity<Student>().HasQueryFilter(x => x.TenantId == this.TenantId && !x.IsDeleted);
			modelBuilder.Entity<Studio>().HasQueryFilter(x => x.TenantId == this.TenantId && !x.IsDeleted);
			modelBuilder.Entity<Teacher>().HasQueryFilter(x => x.TenantId == this.TenantId && !x.IsDeleted);
			modelBuilder.Entity<TeacherSkill>().HasQueryFilter(x => x.TenantId == this.TenantId && !x.IsDeleted);
			modelBuilder.Entity<TeacherTeacherSkill>().HasQueryFilter(x => x.TenantId == this.TenantId && !x.IsDeleted);
			modelBuilder.Entity<User>().HasQueryFilter(x => x.TenantId == this.TenantId && !x.IsDeleted);
	...
	public class ApplicationDbContextFactory : IDesignTimeDbContextFactory<ApplicationDbContext>
	{
		...
		{
			string settingsDirectory = Path.Combine(Directory.GetParent(Directory.GetCurrentDirectory()).FullName, "StudioResourceManagerMT.Api.Web");
