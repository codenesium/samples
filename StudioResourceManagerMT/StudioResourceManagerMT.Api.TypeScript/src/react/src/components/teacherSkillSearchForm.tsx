...
interface TeacherSkillSearchComponentState
{
    records:Array<TeacherSkillViewModel>;
...
export default class TeacherSkillSearchComponent extends React.Component<TeacherSkillSearchComponentProps, TeacherSkillSearchComponentState> {

    state = ({deleteSubmitted:false, deleteSuccess:false, deleteResponse:'', records:new Array<Api.TeacherSkillClientResponseModel>(), filteredRecords:new Array<Api.TeacherSkillClientResponseModel>(), searchValue:'', loading:false, loaded:true, toCreate:false, errorOccurred:false, errorMessage:'', toUpdate:false,toUpdateId:0, toDetail:false,toDetailId:0});
    ...
    loadData () {
        axios.get(Constants.ApiUrl + 'TeacherSkills',
        {
            headers: {
        ...
        .then(resp => {
			...
			let viewModels : Array<TeacherSkillViewModel> = [];
            ...
            let response = error.response.data as Array<Api.TeacherSkillClientResponseModel>;
    ...
    handleDeleteClick(e:any, row:Api.TeacherSkillClientResponseModel) {
        axios.delete(Constants.ApiUrl + 'teacherskills/' + row.id,
        {
            headers: {
                'Content-Type': 'application/json',
            }
   ...
   axios.get(Constants.ApiUrl + 'TeacherSkills?q=' + e.currentTarget.value,
   {
       headers: {
           'Content-Type': 'application/json',
       }
   ...
   .then(resp => {
       ...
       let response = error.response.data as Array<Api.TeacherSkillClientResponseModel>;
       this.setState({records:new Array<Api.TeacherSkillClientResponseModel>(),filteredRecords:new Array<Api.TeacherSkillClientResponseModel>(), loading:false, loaded:false, errorOccurred:true, errorMessage:'Error from API'});
       console.log(response);
   })
    }
    ...
    render () {
        if(this.state.toCreate) {
            return <Redirect to={'/teacherskills/create'} />
        }
        else if(this.state.toUpdate) {
            return <Redirect to={'/teacherskills/edit/' + this.state.toUpdateId} />
        }
        else if(this.state.toDetail) {
            return <Redirect to={'/teacherskills/' + this.state.toDetailId} />
        }
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.TeacherSkillClientResponseModel)}} >Detail</button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.TeacherSkillClientResponseModel)}} >Edit</button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.TeacherSkillClientResponseModel)}} >Delete</button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>408d80bd97797307fe00954b30d96eb7</Hash>
</Codenesium>*/