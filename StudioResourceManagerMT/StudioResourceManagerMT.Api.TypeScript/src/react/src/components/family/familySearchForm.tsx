...
export default class FamilySearchComponent extends React.Component<FamilySearchComponentProps, FamilySearchComponentState> {
    ...
    state = ({deleteSubmitted:false, deleteSuccess:false, deleteResponse:'', records:new Array<FamilyViewModel>(), filteredRecords:new Array<FamilyViewModel>(), searchValue:'', loading:false, loaded:true, errorOccurred:false, errorMessage:''});
    ...
    handleDeleteClick(e:any, row:Api.FamilyClientResponseModel) {
        axios.delete(Constants.ApiEndpoint + ApiRoutes.Families + '/' + row.id,
        {
            headers: {
   ...
   loadRecords(query:string = '') {
	   ...
	   let searchEndpoint = Constants.ApiEndpoint + ApiRoutes.Families + '?limit=100';
        ...
        if(this.state.loading) {
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.FamilyClientResponseModel)}} ><i className="fas fa-search"></i></button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.FamilyClientResponseModel)}} ><i className="fas fa-edit"></i></button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.FamilyClientResponseModel)}} ><i className="far fa-trash-alt"></i></button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>1f8fd3aa46c716bdeee3ff9dff2db03c</Hash>
</Codenesium>*/