...
export default class UserSearchComponent extends React.Component<UserSearchComponentProps, UserSearchComponentState> {
    ...
    state = ({deleteSubmitted:false, deleteSuccess:false, deleteResponse:'', records:new Array<Api.UserClientResponseModel>(), filteredRecords:new Array<Api.UserClientResponseModel>(), searchValue:'', loading:false, loaded:true, toCreate:false, errorOccurred:false, errorMessage:'', toUpdate:false,toUpdateId:0, toDetail:false,toDetailId:0});
    ...
    loadData () {
        ...
        .then(resp => {
            ...
            let response = error.response.data as Array<Api.UserClientResponseModel>;
    ...
    handleDeleteClick(e:any, row:Api.UserClientResponseModel) {
        axios.delete(Constants.ApiUrl + 'users/' + row.id,
        {
            headers: {
                'Content-Type': 'application/json',
            }
   ...
   axios.get(Constants.ApiUrl + 'Users?q=' + e.currentTarget.value,
   {
       headers: {
           'Content-Type': 'application/json',
       }
   ...
   .then(resp => {
       ...
       let response = error.response.data as Array<Api.UserClientResponseModel>;
       this.setState({records:new Array<Api.UserClientResponseModel>(),filteredRecords:new Array<Api.UserClientResponseModel>(), loading:false, loaded:false, errorOccurred:true, errorMessage:'Error from API'});
       console.log(response);
   })
    }
    ...
    render () {
        if(this.state.toCreate) {
            ...
            return <Redirect to={'/users/edit/' + this.state.toUpdateId} />
        }
        else if(this.state.toDetail) {
            return <Redirect to={'/users/' + this.state.toDetailId} />
        }
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.UserClientResponseModel)}} >Detail</button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.UserClientResponseModel)}} >Edit</button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.UserClientResponseModel)}} >Delete</button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>a66f346cf3104f9a65abdbb3d7a9e092</Hash>
</Codenesium>*/