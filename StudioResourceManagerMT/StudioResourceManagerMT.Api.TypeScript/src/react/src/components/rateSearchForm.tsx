...
export default class RateSearchComponent extends React.Component<RateSearchComponentProps, RateSearchComponentState> {
    ...
    state = ({deleteSubmitted:false, deleteSuccess:false, deleteResponse:'', records:new Array<Api.RateClientResponseModel>(), filteredRecords:new Array<Api.RateClientResponseModel>(), searchValue:'', loading:false, loaded:true, toCreate:false, errorOccurred:false, errorMessage:'', toUpdate:false,toUpdateId:0, toDetail:false,toDetailId:0});
    ...
    loadData () {
        axios.get(Constants.ApiUrl + 'Rates',
        {
            headers: {
        ...
        .then(resp => {
            ...
            let response = error.response.data as Array<Api.RateClientResponseModel>;
    ...
    handleDeleteClick(e:any, row:Api.RateClientResponseModel) {
        axios.delete(Constants.ApiUrl + 'rates/' + row.id,
        {
            headers: {
                'Content-Type': 'application/json',
            }
   ...
   axios.get(Constants.ApiUrl + 'Rates?q=' + e.currentTarget.value,
   {
       headers: {
           'Content-Type': 'application/json',
       }
   ...
   .then(resp => {
       ...
       let response = error.response.data as Array<Api.RateClientResponseModel>;
       this.setState({records:new Array<Api.RateClientResponseModel>(),filteredRecords:new Array<Api.RateClientResponseModel>(), loading:false, loaded:false, errorOccurred:true, errorMessage:'Error from API'});
       console.log(response);
   })
    }
    ...
    render () {
        if(this.state.toCreate) {
            return <Redirect to={'/rates/create'} />
        }
        else if(this.state.toUpdate) {
            return <Redirect to={'/rates/edit/' + this.state.toUpdateId} />
        }
        else if(this.state.toDetail) {
            return <Redirect to={'/rates/' + this.state.toDetailId} />
        }
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.RateClientResponseModel)}} >Detail</button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.RateClientResponseModel)}} >Edit</button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.RateClientResponseModel)}} >Delete</button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>9d098b07a04fbd617df73dcf06183e8f</Hash>
</Codenesium>*/