...
export default class DeviceActionSearchComponent extends React.Component<DeviceActionSearchComponentProps, DeviceActionSearchComponentState> {

    state = ({deleteSubmitted:false, deleteSuccess:false, deleteResponse:'', records:new Array<DeviceActionViewModel>(), filteredRecords:new Array<DeviceActionViewModel>(), searchValue:'', loading:false, loaded:true, errorOccurred:false, errorMessage:''});
    ...
    handleDeleteClick(e:any, row:Api.DeviceActionClientResponseModel) {
        axios.delete(Constants.ApiEndpoint + ApiRoutes.DeviceActions + '/' + row.id,
        {
            headers: {
   ...
   loadRecords(query:string = '') {
	   ...
	   let searchEndpoint = Constants.ApiEndpoint + ApiRoutes.DeviceActions + '?limit=100';
	   ...
	   .then(resp => {
		   ...
		   console.log(error);
		   this.setState({records:new Array<DeviceActionViewModel>(),filteredRecords:new Array<DeviceActionViewModel>(), loading:false, loaded:false, errorOccurred:true, errorMessage:'Error from API'});
	   })
    }
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    columns: [
                      ...
                      accessor: 'deviceId',
                      Cell: (props) => {
                        return <a href='' onClick={(e) => { e.preventDefault(); this.props.history.push(ClientRoutes.Devices + '/' + props.original.deviceId); }}>
                          {String(
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.DeviceActionClientResponseModel)}} ><i className="fas fa-search"></i></button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.DeviceActionClientResponseModel)}} ><i className="fas fa-edit"></i></button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.DeviceActionClientResponseModel)}} ><i className="far fa-trash-alt"></i></button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>cefac2f29733a6341bd8bbb7d8089c14</Hash>
</Codenesium>*/