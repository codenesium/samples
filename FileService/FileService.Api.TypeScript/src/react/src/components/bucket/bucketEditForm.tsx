...
interface Props {
  ...
  const BucketEditDisplay = (props: FormikProps<BucketViewModel>) => {
   ...
   { 
        let response = '';
        if(props.touched[name as keyof BucketViewModel]  && props.errors[name as keyof BucketViewModel]) {
...
const BucketEdit = withFormik<Props, BucketViewModel>({
    mapPropsToValues: props => {
		...
		response.setProperties(props.model!.externalId,props.model!.id,props.model!.name);	
		return response;
      },
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.put(Constants.ApiEndpoint + ApiRoutes.Buckets +'/' + values.id,
        ...
        .then(resp => {
            let response = resp.data as UpdateResponse<Api.BucketClientRequestModel>;
  ...
  export default class BucketEditComponent extends React.Component<BucketEditComponentProps, BucketEditComponentState> {
    ...
    componentDidMount () {
        ...
        axios.get(Constants.ApiEndpoint + ApiRoutes.Buckets + '/' + this.props.match.params.id, {
            headers: {
                'Content-Type': 'application/json',
            }
        })
...
/*<Codenesium>
    <Hash>dc7df991d3886ff9213340d8e3063c24</Hash>
</Codenesium>*/