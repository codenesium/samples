...
WORKDIR FileService.Api.TypeScript/src/react
...
RUN dotnet publish FileService.Api.Web/FileService.Api.Web.csproj -c Release  -o /publish -r alpine-x64 --self-contained
...
COPY --from=jsbuild FileService.Api.TypeScript/src/react/build .

WORKDIR /publish
ENTRYPOINT ["./FileService.Api.Web"]