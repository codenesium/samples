...
namespace sample1NS.Api.DataAccess
{
	public abstract class AbstractLinkStatusRepository
	{
		...
		private List<LinkStatus> SearchLinqEF(Expression<Func<LinkStatus, bool>> predicate,int skip=0,int take=Int32.MaxValue,string orderClause="")
		{
		...
		private List<LinkStatus> SearchLinqEFDynamic(string predicate,int skip=0,int take=Int32.MaxValue,string orderClause="")
		{
			if(String.IsNullOrEmpty(orderClause))
			{
				return this._context.Set<LinkStatus>().Where(predicate).AsQueryable().OrderBy("id ASC").Skip(skip).Take(take).ToList<LinkStatus>();
...
/*<Codenesium>
    <Hash>9048bbfc0f38876dbc8abb15dfd4494e</Hash>
</Codenesium>*/