...
WORKDIR PointOfSale.Api.TypeScript/src/react
...
RUN dotnet publish PointOfSale.Api.Web/PointOfSale.Api.Web.csproj -c Release  -o /publish -r alpine-x64 --self-contained
...
COPY --from=jsbuild PointOfSale.Api.TypeScript/src/react/build .

WORKDIR /publish
ENTRYPOINT ["./PointOfSale.Api.Web"]