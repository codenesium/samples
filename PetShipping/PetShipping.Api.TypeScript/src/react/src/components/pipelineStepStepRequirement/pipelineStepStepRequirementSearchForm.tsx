...
export default class PipelineStepStepRequirementSearchComponent extends React.Component<PipelineStepStepRequirementSearchComponentProps, PipelineStepStepRequirementSearchComponentState> {
    ...
    handleEditClick(e:any, row:Api.PipelineStepStepRequirementClientResponseModel) {
         this.props.history.push(ClientRoutes.PipelineStepStepRequirements + '/edit/' + row.id);
    }

    handleDetailClick(e:any, row:Api.PipelineStepStepRequirementClientResponseModel) {
   ...
   loadRecords(query:string = '') {
	   ...
	   .then(resp => {
		   ...
		   console.log(error);
		   this.setState({records:new Array<PipelineStepStepRequirementViewModel>(),filteredRecords:new Array<PipelineStepStepRequirementViewModel>(), loading:false, loaded:false, errorOccurred:true, errorMessage:'Error from API'});
	   })
    }
    ...
    render () {
        if(this.state.loading) {
            ...
            <ReactTable 
                ...
                columns={[{
                    Header: 'PipelineStepStepRequirement',
                    columns: [
					  {
                      Header: 'Details',
                      accessor: 'detail',
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.PipelineStepStepRequirementClientResponseModel)}} ><i className="fas fa-search"></i></button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.PipelineStepStepRequirementClientResponseModel)}} ><i className="fas fa-edit"></i></button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.PipelineStepStepRequirementClientResponseModel)}} ><i className="far fa-trash-alt"></i></button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>48ae630d5711358ea4a6799bce8b07dc</Hash>
</Codenesium>*/