...
interface Props {
  ...
  const PipelineStepNoteEditDisplay = (props: FormikProps<PipelineStepNoteViewModel>) => {

   let status = props.status as UpdateResponse<Api.PipelineStepNoteClientRequestModel>;
   
   let errorsForField = (name:string) : string =>
   { 
        let response = '';
        if(props.touched[name as keyof PipelineStepNoteViewModel]  && props.errors[name as keyof PipelineStepNoteViewModel]) {
...
const PipelineStepNoteEdit = withFormik<Props, PipelineStepNoteViewModel>({
    mapPropsToValues: props => {
		...
		response.setProperties(props.model!.employeeId,props.model!.id,props.model!.note,props.model!.pipelineStepId);	
		return response;
      },
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.put(Constants.ApiEndpoint + ApiRoutes.PipelineStepNotes +'/' + values.id,
        ...
        .then(resp => {
            let response = resp.data as UpdateResponse<Api.PipelineStepNoteClientRequestModel>;
  ...
  export default class PipelineStepNoteEditComponent extends React.Component<PipelineStepNoteEditComponentProps, PipelineStepNoteEditComponentState> {
    ...
    componentDidMount () {
        ...
        axios.get(Constants.ApiEndpoint + ApiRoutes.PipelineStepNotes + '/' + this.props.match.params.id, {
            headers: {
                'Content-Type': 'application/json',
            }
        })
...
/*<Codenesium>
    <Hash>62b26f77724e787b4a4db39138c5ec4f</Hash>
</Codenesium>*/