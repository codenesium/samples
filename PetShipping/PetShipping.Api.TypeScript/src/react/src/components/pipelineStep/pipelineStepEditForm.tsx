...
interface Props {
  ...
  const PipelineStepEditDisplay = (props: FormikProps<PipelineStepViewModel>) => {
   ...
   let status = props.status as UpdateResponse<Api.PipelineStepClientRequestModel>;
...
const PipelineStepEdit = withFormik<Props, PipelineStepViewModel>({
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.put(Constants.ApiEndpoint + ApiRoutes.PipelineSteps +'/' + values.id,
        ...
        .then(resp => {
            let response = resp.data as UpdateResponse<Api.PipelineStepClientRequestModel>;
  ...
  export default class PipelineStepEditComponent extends React.Component<PipelineStepEditComponentProps, PipelineStepEditComponentState> {
    ...
    componentDidMount () {
        ...
        axios.get(Constants.ApiEndpoint + ApiRoutes.PipelineSteps + '/' + this.props.match.params.id, {
            headers: {
                'Content-Type': 'application/json',
            }
        })
...
/*<Codenesium>
    <Hash>1418b6a5544f04b3d215c23dec9e2655</Hash>
</Codenesium>*/