...
interface Props {
  ...
  const PipelineStepDestinationEditDisplay = (props: FormikProps<PipelineStepDestinationViewModel>) => {

   let status = props.status as UpdateResponse<Api.PipelineStepDestinationClientRequestModel>;
   
   let errorsForField = (name:string) : string =>
   { 
        let response = '';
        if(props.touched[name as keyof PipelineStepDestinationViewModel]  && props.errors[name as keyof PipelineStepDestinationViewModel]) {
   ...
   return (
          ...
          <form onSubmit={props.handleSubmit} role="form">
                        ...
                        <label htmlFor="name" className={errorExistForField("destinationId") ? ("col-sm-2 col-form-label is-invalid") : "col-sm-2 col-form-label"}>DestinationId</label>
					    <div className="col-sm-12">
                             <Field type="datetime-local" name="destinationId" className={errorExistForField("destinationId") ? "form-control is-invalid" : "form-control"} />
                            {errorExistForField("destinationId") && <small className="text-danger">{errorsForField("destinationId")}</small>}
                        </div>
                    </div>
							<div className="form-group row">
...
const PipelineStepDestinationEdit = withFormik<Props, PipelineStepDestinationViewModel>({
    mapPropsToValues: props => {
        let response = new PipelineStepDestinationViewModel();
		response.setProperties(props.model!.destinationId,props.model!.id,props.model!.pipelineStepId);	
		return response;
      },
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.put(Constants.ApiEndpoint + ApiRoutes.PipelineStepDestinations +'/' + values.id,
        ...
        .then(resp => {
            let response = resp.data as UpdateResponse<Api.PipelineStepDestinationClientRequestModel>;
  ...
  export default class PipelineStepDestinationEditComponent extends React.Component<PipelineStepDestinationEditComponentProps, PipelineStepDestinationEditComponentState> {
    ...
    componentDidMount () {
        ...
        axios.get(Constants.ApiEndpoint + ApiRoutes.PipelineStepDestinations + '/' + this.props.match.params.id, {
            headers: {
                'Content-Type': 'application/json',
            }
        })
...
/*<Codenesium>
    <Hash>cf2374486b4cc620cfdddd4a251c7896</Hash>
</Codenesium>*/