...
export default class HandlerPipelineStepSearchComponent extends React.Component<HandlerPipelineStepSearchComponentProps, HandlerPipelineStepSearchComponentState> {
    ...
    handleEditClick(e:any, row:Api.HandlerPipelineStepClientResponseModel) {
         this.props.history.push(ClientRoutes.HandlerPipelineSteps + '/edit/' + row.id);
    }

    handleDetailClick(e:any, row:Api.HandlerPipelineStepClientResponseModel) {
   ...
   loadRecords(query:string = '') {
	   ...
	   .then(resp => {
		   ...
		   console.log(error);
		   this.setState({records:new Array<HandlerPipelineStepViewModel>(),filteredRecords:new Array<HandlerPipelineStepViewModel>(), loading:false, loaded:false, errorOccurred:true, errorMessage:'Error from API'});
	   })
    }
        ...
        if(this.state.loading) {
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.HandlerPipelineStepClientResponseModel)}} ><i className="fas fa-search"></i></button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.HandlerPipelineStepClientResponseModel)}} ><i className="fas fa-edit"></i></button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.HandlerPipelineStepClientResponseModel)}} ><i className="far fa-trash-alt"></i></button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>68fa5c51b875e58c9f3fa8b968accf0c</Hash>
</Codenesium>*/