...
interface Props {
  ...
  const EmployeeEditDisplay = (props: FormikProps<EmployeeViewModel>) => {
   ...
   { 
        let response = '';
        if(props.touched[name as keyof EmployeeViewModel]  && props.errors[name as keyof EmployeeViewModel]) {
   ...
   return (
          ...
          <form onSubmit={props.handleSubmit} role="form">
					    ...
					    <div className="col-sm-12">
							...
							<div className="form-group row">
                        <label htmlFor="name" className={errorExistForField("isSalesPerson") ? ("col-sm-2 col-form-label is-invalid") : "col-sm-2 col-form-label"}>IsSalesPerson</label>
					    <div className="col-sm-12">
                             <Field type="datetime-local" name="isSalesPerson" className={errorExistForField("isSalesPerson") ? "form-control is-invalid" : "form-control"} />
                            {errorExistForField("isSalesPerson") && <small className="text-danger">{errorsForField("isSalesPerson")}</small>}
                        </div>
                    </div>
							<div className="form-group row">
                        <label htmlFor="name" className={errorExistForField("isShipper") ? ("col-sm-2 col-form-label is-invalid") : "col-sm-2 col-form-label"}>IsShipper</label>
					    <div className="col-sm-12">
                             <Field type="datetime-local" name="isShipper" className={errorExistForField("isShipper") ? "form-control is-invalid" : "form-control"} />
                            {errorExistForField("isShipper") && <small className="text-danger">{errorsForField("isShipper")}</small>}
                        </div>
                    </div>
							<div className="form-group row">
...
const EmployeeEdit = withFormik<Props, EmployeeViewModel>({
    mapPropsToValues: props => {
		...
		response.setProperties(props.model!.firstName,props.model!.id,props.model!.isSalesPerson,props.model!.isShipper,props.model!.lastName);	
		return response;
      },
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.put(Constants.ApiEndpoint + ApiRoutes.Employees +'/' + values.id,
        ...
        .then(resp => {
            let response = resp.data as UpdateResponse<Api.EmployeeClientRequestModel>;
  ...
  export default class EmployeeEditComponent extends React.Component<EmployeeEditComponentProps, EmployeeEditComponentState> {
    ...
    componentDidMount () {
        ...
        axios.get(Constants.ApiEndpoint + ApiRoutes.Employees + '/' + this.props.match.params.id, {
            headers: {
                'Content-Type': 'application/json',
            }
        })
...
/*<Codenesium>
    <Hash>299e308f8cb6f354247a48cc01ae5785</Hash>
</Codenesium>*/