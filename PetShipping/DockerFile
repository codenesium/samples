...
WORKDIR PetShipping.Api.TypeScript/src/react
...
RUN dotnet publish PetShipping.Api.Web/PetShipping.Api.Web.csproj -c Release  -o /publish -r alpine-x64 --self-contained
...
COPY --from=jsbuild PetShipping.Api.TypeScript/src/react/build .

WORKDIR /publish
ENTRYPOINT ["./PetShipping.Api.Web"]