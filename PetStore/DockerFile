...
WORKDIR PetStore.Api.TypeScript/src/react
...
RUN dotnet publish PetStore.Api.Web/PetStore.Api.Web.csproj -c Release  -o /publish -r alpine-x64 --self-contained
...
COPY --from=jsbuild PetStore.Api.TypeScript/src/react/build .

WORKDIR /publish
ENTRYPOINT ["./PetStore.Api.Web"]