...
WORKDIR SecureVideoCRM.Api.TypeScript/src/react
...
RUN dotnet publish SecureVideoCRM.Api.Web/SecureVideoCRM.Api.Web.csproj -c Release  -o /publish -r alpine-x64 --self-contained
...
COPY --from=jsbuild SecureVideoCRM.Api.TypeScript/src/react/build .

WORKDIR /publish
ENTRYPOINT ["./SecureVideoCRM.Api.Web"]