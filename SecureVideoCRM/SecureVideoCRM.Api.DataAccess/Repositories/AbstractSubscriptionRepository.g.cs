...
namespace SecureVideoCRMNS.Api.DataAccess
{
	public abstract class AbstractSubscriptionRepository : AbstractRepository
	{
		...
		// Foreign key reference to this table User via subscriptionId.
		public async virtual Task<List<User>> UsersBySubscriptionId(int subscriptionId, int limit = int.MaxValue, int offset = 0)
		{
			return await this.Context.Set<User>()
			       .Include(x => x.SubscriptionIdNavigation)
			       .Where(x => x.SubscriptionId == subscriptionId).AsQueryable().Skip(offset).Take(limit).ToListAsync<User>();
		}
...
/*<Codenesium>
    <Hash>55d9378bfa8f344762c7bf4286a6bcb7</Hash>
</Codenesium>*/