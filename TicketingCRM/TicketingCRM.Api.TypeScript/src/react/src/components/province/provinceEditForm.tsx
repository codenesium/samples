...
interface Props {
  ...
  const ProvinceEditDisplay = (props: FormikProps<ProvinceViewModel>) => {

   let status = props.status as UpdateResponse<Api.ProvinceClientRequestModel>;
   
   let errorsForField = (name:string) : string =>
   { 
        let response = '';
        if(props.touched[name as keyof ProvinceViewModel]  && props.errors[name as keyof ProvinceViewModel]) {
...
const ProvinceEdit = withFormik<Props, ProvinceViewModel>({
    mapPropsToValues: props => {
        let response = new ProvinceViewModel();
		response.setProperties(props.model!.countryId,props.model!.id,props.model!.name);	
		return response;
      },
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.put(Constants.ApiEndpoint + ApiRoutes.Provinces +'/' + values.id,
        ...
        .then(resp => {
            let response = resp.data as UpdateResponse<Api.ProvinceClientRequestModel>;
  ...
  export default class ProvinceEditComponent extends React.Component<ProvinceEditComponentProps, ProvinceEditComponentState> {
    ...
    componentDidMount () {
        ...
        axios.get(Constants.ApiEndpoint + ApiRoutes.Provinces + '/' + this.props.match.params.id, {
            headers: {
                'Content-Type': 'application/json',
            }
        })
    ...
    render () {
        ...
        if (this.state.loading) {
            ...
            return (<ProvinceEdit model={this.state.model} />);
        } 
		else {
		  return null;
		}
...
/*<Codenesium>
    <Hash>b83bf6a5ef4c6479e76bb55336b81d2e</Hash>
</Codenesium>*/