...
interface ProvinceSearchComponentState
{
    records:Array<ProvinceViewModel>;
...
export default class ProvinceSearchComponent extends React.Component<ProvinceSearchComponentProps, ProvinceSearchComponentState> {

    state = ({deleteSubmitted:false, deleteSuccess:false, deleteResponse:'', records:new Array<ProvinceViewModel>(), filteredRecords:new Array<ProvinceViewModel>(), searchValue:'', loading:false, loaded:true, errorOccurred:false, errorMessage:''});
    ...
    handleCreateClick(e:any) {
        this.props.history.push(ClientRoutes.Provinces + '/create');
    }

    handleDeleteClick(e:any, row:Api.ProvinceClientResponseModel) {
        axios.delete(Constants.ApiEndpoint + ApiRoutes.Provinces + '/' + row.id,
        {
            headers: {
   ...
   loadRecords(query:string = '') {
	   ...
	   let searchEndpoint = Constants.ApiEndpoint + ApiRoutes.Provinces + '?limit=100';
	   ...
	   .then(resp => {
		    ...
		    let viewModels : Array<ProvinceViewModel> = [];
        ...
        if(this.state.loading) {
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.ProvinceClientResponseModel)}} ><i className="fas fa-search"></i></button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.ProvinceClientResponseModel)}} ><i className="fas fa-edit"></i></button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.ProvinceClientResponseModel)}} ><i className="far fa-trash-alt"></i></button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>1510f13003c3018ba788a7eec588ce27</Hash>
</Codenesium>*/