...
import VenueViewModel from  './venueViewModel';
	...
	mapApiResponseToViewModel(dto: Api.VenueClientResponseModel) : VenueViewModel 
	{
		...
		response.setProperties(dto.address1,dto.address2,dto.adminId,dto.email,dto.facebook,dto.id,dto.name,dto.phone,dto.provinceId,dto.website);
		
						if(dto.adminIdNavigation != null)
				{
				  response.adminIdNavigation = new AdminViewModel();
				  response.adminIdNavigation.setProperties(
				  dto.adminIdNavigation.email,dto.adminIdNavigation.firstName,dto.adminIdNavigation.id,dto.adminIdNavigation.lastName,dto.adminIdNavigation.password,dto.adminIdNavigation.phone,dto.adminIdNavigation.username
				  );
				}
	...
	mapViewModelToApiRequest(model: VenueViewModel) : Api.VenueClientRequestModel
	{
		...
		response.setProperties(model.address1,model.address2,model.adminId,model.email,model.facebook,model.id,model.name,model.phone,model.provinceId,model.website);
		return response;
	}
};

/*<Codenesium>
    <Hash>4a3b787430611b16f3f0247522b14d44</Hash>
</Codenesium>*/