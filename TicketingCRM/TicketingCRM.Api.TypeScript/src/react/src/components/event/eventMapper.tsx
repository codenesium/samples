...
import EventViewModel from  './eventViewModel';
	...
	mapApiResponseToViewModel(dto: Api.EventClientResponseModel) : EventViewModel 
	{
		...
		response.setProperties(dto.address1,dto.address2,dto.cityId,dto.date,dto.description,dto.endDate,dto.facebook,dto.id,dto.name,dto.startDate,dto.website);
		
						if(dto.cityIdNavigation != null)
				{
				  response.cityIdNavigation = new CityViewModel();
				  response.cityIdNavigation.setProperties(
				  dto.cityIdNavigation.id,dto.cityIdNavigation.name,dto.cityIdNavigation.provinceId
				  );
				}
	...
	mapViewModelToApiRequest(model: EventViewModel) : Api.EventClientRequestModel
	{
		...
		response.setProperties(model.address1,model.address2,model.cityId,model.date,model.description,model.endDate,model.facebook,model.id,model.name,model.startDate,model.website);
		return response;
	}
};

/*<Codenesium>
    <Hash>4e3707687a412dbdd447d3615a08a0f5</Hash>
</Codenesium>*/