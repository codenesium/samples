...
interface Props {
  ...
  const CityEditDisplay = (props: FormikProps<CityViewModel>) => {

   let status = props.status as UpdateResponse<Api.CityClientRequestModel>;
   
   let errorsForField = (name:string) : string =>
   { 
        let response = '';
        if(props.touched[name as keyof CityViewModel]  && props.errors[name as keyof CityViewModel]) {
...
const CityEdit = withFormik<Props, CityViewModel>({
    mapPropsToValues: props => {
        let response = new CityViewModel();
		response.setProperties(props.model!.id,props.model!.name,props.model!.provinceId);	
		return response;
      },
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.put(Constants.ApiEndpoint + ApiRoutes.Cities +'/' + values.id,
        ...
        .then(resp => {
            let response = resp.data as UpdateResponse<Api.CityClientRequestModel>;
  ...
  export default class CityEditComponent extends React.Component<CityEditComponentProps, CityEditComponentState> {
    ...
    render () {
        ...
        if (this.state.loading) {
            ...
            return (<CityEdit model={this.state.model} />);
        } 
		else {
		  return null;
		}
...
/*<Codenesium>
    <Hash>21ea5e469fbc05a43a264d83373f5acb</Hash>
</Codenesium>*/