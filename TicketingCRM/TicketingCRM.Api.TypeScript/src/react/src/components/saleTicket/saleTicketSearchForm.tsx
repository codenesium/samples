...
export default class SaleTicketSearchComponent extends React.Component<SaleTicketSearchComponentProps, SaleTicketSearchComponentState> {
    ...
    handleEditClick(e:any, row:Api.SaleTicketClientResponseModel) {
         this.props.history.push(ClientRoutes.SaleTickets + '/edit/' + row.id);
    }

    handleDetailClick(e:any, row:Api.SaleTicketClientResponseModel) {
   ...
   loadRecords(query:string = '') {
	   ...
	   .then(resp => {
		   ...
		   console.log(error);
		   this.setState({records:new Array<SaleTicketViewModel>(),filteredRecords:new Array<SaleTicketViewModel>(), loading:false, loaded:false, errorOccurred:true, errorMessage:'Error from API'});
	   })
    }
        ...
        if(this.state.loading) {
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.SaleTicketClientResponseModel)}} ><i className="fas fa-search"></i></button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.SaleTicketClientResponseModel)}} ><i className="fas fa-edit"></i></button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.SaleTicketClientResponseModel)}} ><i className="far fa-trash-alt"></i></button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>aa481433510616fae20002df5da80555</Hash>
</Codenesium>*/