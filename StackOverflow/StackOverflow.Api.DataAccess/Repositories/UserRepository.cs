...
namespace StackOverflowNS.Api.DataAccess
{
	public class UserRepository : AbstractRepository, IUserRepository
	{
		...
		public virtual Task<List<User>> All(int limit = int.MaxValue, int offset = 0, string query = "")
		{
			...
			{
				return this.Where(x =>
				                  (x.AboutMe == null?false : x.AboutMe.StartsWith(query)) ||
				                  x.AccountId == query.ToNullableInt() ||
				                  x.Age == query.ToNullableInt() ||
				                  x.CreationDate == query.ToDateTime() ||
				                  (x.DisplayName == null?false : x.DisplayName.StartsWith(query)) ||
				                  x.DownVote == query.ToInt() ||
				                  (x.EmailHash == null?false : x.EmailHash.StartsWith(query)) ||
				                  ...
				                  x.View == query.ToInt() ||
				                  (x.WebsiteUrl == null?false : x.WebsiteUrl.StartsWith(query)),
				                  limit,
				                  offset);
			}
		}
...
/*<Codenesium>
    <Hash>3a996f1e14aad098273d907794a88276</Hash>
    <Hello>
