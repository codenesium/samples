...
WORKDIR StackOverflow.Api.TypeScript/src/react
...
RUN dotnet publish StackOverflow.Api.Web/StackOverflow.Api.Web.csproj -c Release  -o /publish -r alpine-x64 --self-contained
...
COPY --from=jsbuild StackOverflow.Api.TypeScript/src/react/build .

WORKDIR /publish
ENTRYPOINT ["./StackOverflow.Api.Web"]