...
interface Props {
  ...
  const VPersonEditDisplay = (props: FormikProps<VPersonViewModel>) => {
   ...
   { 
        let response = '';
        if(props.touched[name as keyof VPersonViewModel]  && props.errors[name as keyof VPersonViewModel]) {
...
const VPersonEdit = withFormik<Props, VPersonViewModel>({
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.put(Constants.ApiEndpoint + ApiRoutes.VPersons +'/' + values.personId,
        ...
        .then(resp => {
            let response = resp.data as UpdateResponse<Api.VPersonClientRequestModel>;
  ...
  export default class VPersonEditComponent extends React.Component<VPersonEditComponentProps, VPersonEditComponentState> {
...
/*<Codenesium>
    <Hash>a0380ad0dde9cdedcd56d806f5b21dae</Hash>
</Codenesium>*/