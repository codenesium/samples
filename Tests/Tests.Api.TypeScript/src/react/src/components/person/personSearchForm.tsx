...
export default class PersonSearchComponent extends React.Component<PersonSearchComponentProps, PersonSearchComponentState> {
    ...
    handleDeleteClick(e:any, row:Api.PersonClientResponseModel) {
        axios.delete(Constants.ApiEndpoint + ApiRoutes.People + '/' + row.personId,
        {
            headers: {
   ...
   loadRecords(query:string = '') {
	   ...
	   .then(resp => {
		   ...
		   console.log(error);
		   this.setState({records:new Array<PersonViewModel>(),filteredRecords:new Array<PersonViewModel>(), loading:false, loaded:false, errorOccurred:true, errorMessage:'Error from API'});
	   })
    }
        ...
        if(this.state.loading) {
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.PersonClientResponseModel)}} ><i className="fas fa-search"></i></button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.PersonClientResponseModel)}} ><i className="fas fa-edit"></i></button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.PersonClientResponseModel)}} ><i className="far fa-trash-alt"></i></button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>cfb49d12bc65973e492711348c122fb4</Hash>
</Codenesium>*/