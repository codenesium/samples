...
interface Props {
  ...
  const TimestampCheckEditDisplay = (props: FormikProps<TimestampCheckViewModel>) => {

   let status = props.status as UpdateResponse<Api.TimestampCheckClientRequestModel>;
...
const TimestampCheckEdit = withFormik<Props, TimestampCheckViewModel>({
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.put(Constants.ApiEndpoint + ApiRoutes.TimestampChecks +'/' + values.id,
        ...
        .then(resp => {
            let response = resp.data as UpdateResponse<Api.TimestampCheckClientRequestModel>;
    ...
    displayName: 'TimestampCheckEdit', 
  })(TimestampCheckEditDisplay);
  ...
  interface TimestampCheckEditComponentState
  {
      model?:TimestampCheckViewModel;
      loading:boolean;
      loaded:boolean;
  ...
  export default class TimestampCheckEditComponent extends React.Component<TimestampCheckEditComponentProps, TimestampCheckEditComponentState> {
    ...
    render () {
        ...
        if (this.state.loading) {
            ...
            return (<TimestampCheckEdit model={this.state.model} />);
        } 
		else {
		  return null;
		}
...
/*<Codenesium>
    <Hash>e8b0ffa3a59e6cc2eadc0f98dff15c8a</Hash>
</Codenesium>*/