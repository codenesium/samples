...
export default class IncludedColumnTestSearchComponent extends React.Component<
  IncludedColumnTestSearchComponentProps,
  IncludedColumnTestSearchComponentState
> {
  state = {
  ...
  handleDeleteClick(e: any, row: Api.IncludedColumnTestClientResponseModel) {
    axios
      .delete(
        Constants.ApiEndpoint + ApiRoutes.IncludedColumnTests + '/' + row.id,
        {
  ...
  loadRecords(query: string = '') {
    ...
    let searchEndpoint =
      Constants.ApiEndpoint + ApiRoutes.IncludedColumnTests + '?limit=100';
    ...
    axios
      .get<Array<Api.IncludedColumnTestClientResponseModel>>(searchEndpoint, {
        headers: GlobalUtilities.defaultHeaders(),
      })
      .then(response => {
...
export const WrappedIncludedColumnTestSearchComponent = Form.create({
  name: 'IncludedColumnTest Search',
})(IncludedColumnTestSearchComponent);
