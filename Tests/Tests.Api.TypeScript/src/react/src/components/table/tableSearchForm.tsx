...
export default class TableSearchComponent extends React.Component<
  TableSearchComponentProps,
  TableSearchComponentState
> {
  state = {
  ...
  handleDeleteClick(e: any, row: Api.TableClientResponseModel) {
    axios
      .delete(Constants.ApiEndpoint + ApiRoutes.Tables + '/' + row.id, {
  ...
  loadRecords(query: string = '') {
    ...
    let searchEndpoint =
      Constants.ApiEndpoint + ApiRoutes.Tables + '?limit=100';
    ...
    axios
      .get<Array<Api.TableClientResponseModel>>(searchEndpoint, {
        headers: GlobalUtilities.defaultHeaders(),
      })
      .then(response => {
...
export const WrappedTableSearchComponent = Form.create({
  name: 'Table Search',
