...
WORKDIR Tests.Api.TypeScript/src/react
...
RUN dotnet publish Tests.Api.Web/Tests.Api.Web.csproj -c Release  -o /publish -r alpine-x64 --self-contained
...
COPY --from=jsbuild Tests.Api.TypeScript/src/react/build .

WORKDIR /publish
ENTRYPOINT ["./Tests.Api.Web"]