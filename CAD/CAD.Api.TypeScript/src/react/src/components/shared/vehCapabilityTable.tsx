...
interface VehCapabilityTableComponentProps {
  apiRoute: string;
  history: any;
  match: any;
}

interface VehCapabilityTableComponentState {
  loading: boolean;
  loaded: boolean;
...
export class VehCapabilityTableComponent extends React.Component<
  VehCapabilityTableComponentProps,
  VehCapabilityTableComponentState
> {
  state = {
    loading: false,
    loaded: true,
  ...
  loadRecords() {
    ...
    axios
      ...
      .then(response => {
        ...
        let vehCapabilities: Array<VehCapabilityViewModel> = [];

        response.data.forEach(x => {
          vehCapabilities.push(mapper.mapApiResponseToViewModel(x));
        });

        this.setState({
          ...this.state,
          filteredRecords: vehCapabilities,
          loading: false,
          loaded: true,
          errorOccurred: false,
          errorMessage: '',
