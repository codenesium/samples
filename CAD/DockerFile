...
WORKDIR CAD.Api.TypeScript/src/react
...
RUN dotnet publish CAD.Api.Web/CAD.Api.Web.csproj -c Release  -o /publish -r alpine-x64 --self-contained
...
COPY --from=jsbuild CAD.Api.TypeScript/src/react/build .

WORKDIR /publish
ENTRYPOINT ["./CAD.Api.Web"]