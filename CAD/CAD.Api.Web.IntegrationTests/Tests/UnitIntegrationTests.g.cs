...
namespace CADNS.Api.Web.IntegrationTests
{
	...
	public partial class UnitIntegrationTests
	{
		...
		public virtual async void TestBulkInsert()
		{
			...
			var model2 = new ApiUnitClientRequestModel();
			model2.SetProperties("C");
			var request = new List<ApiUnitClientRequestModel>() {model, model2};
			CreateResponse<List<ApiUnitClientResponseModel>> result = await client.UnitBulkInsertAsync(request);

			result.Success.Should().BeTrue();
			result.Record.Should().NotBeNull();

			context.Set<Unit>().ToList()[1].CallSign.Should().Be("B");

			context.Set<Unit>().ToList()[2].CallSign.Should().Be("C");
		}
		...
		public virtual async void TestCreate()
		{
			...
			var model = new ApiUnitClientRequestModel();
			model.SetProperties("B");
			CreateResponse<ApiUnitClientResponseModel> result = await client.UnitCreateAsync(model);

			result.Success.Should().BeTrue();
			result.Record.Should().NotBeNull();
			context.Set<Unit>().ToList()[1].CallSign.Should().Be("B");

			result.Record.CallSign.Should().Be("B");
		}
		...
		public virtual async void TestGetFound()
		{
			...
			ApiUnitClientResponseModel response = await client.UnitGetAsync(1);
		...
		public virtual async void TestGetNotFound()
		{
			...
			client.SetBearerToken(jwtHelper.GenerateBearerToken(
									  ...
									  "test@test.com",
									  "Passw0rd$"));
			ApiUnitClientResponseModel response = await client.UnitGetAsync(default(int));
		...
		public virtual async void TestAll()
		{
			...
			client.SetBearerToken(jwtHelper.GenerateBearerToken(
									  ...
									  "test@test.com",
									  "Passw0rd$"));
			List<ApiUnitClientResponseModel> response = await client.UnitAllAsync();

			response.Count.Should().BeGreaterThan(0);
			response[0].CallSign.Should().Be("A");
		...
		public virtual void TestClientCancellationToken()
		{
			Func<Task> testCancellation = async () =>
			{
				...
				tokenSource.Cancel();
				var result = await client.UnitAllAsync(token);
			};
...
/*<Codenesium>
    <Hash>7cb021d8bc9fdbc4065c7ece51cc2cb7</Hash>
</Codenesium>*/