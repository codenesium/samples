...
namespace NebulaNS.Api.Web
{
	public abstract class AbstractChainController : AbstractApiController
	{
		...
		public async virtual Task<IActionResult> LinksByChainId(int chainId, int? limit, int? offset)
		{
			...
			List<ApiLinkResponseModel> response = await this.ChainService.LinksByChainId(chainId, query.Limit, query.Offset);

			return this.Ok(response);
		}

		[HttpGet]
		[Route("byPreviousChainId/{previousChainId}")]
		[ReadOnly]
		[ProducesResponseType(typeof(List<ApiChainResponseModel>), 200)]
		public async virtual Task<IActionResult> ByPreviousChainId(int previousChainId, int? limit, int? offset)
		{
			SearchQuery query = new SearchQuery();
			if (!query.Process(this.MaxLimit, this.DefaultLimit, limit, offset, this.ControllerContext.HttpContext.Request.Query.ToDictionary(q => q.Key, q => q.Value)))
			{
			...
			List<ApiChainResponseModel> response = await this.ChainService.ByPreviousChainId(previousChainId, query.Limit, query.Offset);
...
/*<Codenesium>
    <Hash>39943cfe1e370b3e7e9bbbcc263cd27a</Hash>
</Codenesium>*/