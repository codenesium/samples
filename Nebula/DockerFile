...
WORKDIR Nebula.Api.TypeScript/src/react
...
RUN dotnet publish Nebula.Api.Web/Nebula.Api.Web.csproj -c Release  -o /publish -r alpine-x64 --self-contained
...
COPY --from=jsbuild Nebula.Api.TypeScript/src/react/build .

WORKDIR /publish
ENTRYPOINT ["./Nebula.Api.Web"]