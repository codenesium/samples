...
interface Props {
  ...
  const LinkStatusEditDisplay = (props: FormikProps<LinkStatusViewModel>) => {
   ...
   { 
        let response = '';
        if(props.touched[name as keyof LinkStatusViewModel]  && props.errors[name as keyof LinkStatusViewModel]) {
...
const LinkStatusEdit = withFormik<Props, LinkStatusViewModel>({
    ...
    handleSubmit: (values, actions) => {
        ...
        axios.put(Constants.ApiEndpoint + ApiRoutes.LinkStatuses +'/' + values.id,
        ...
        .then(resp => {
            let response = resp.data as UpdateResponse<Api.LinkStatusClientRequestModel>;
  ...
  export default class LinkStatusEditComponent extends React.Component<LinkStatusEditComponentProps, LinkStatusEditComponentState> {
    ...
    componentDidMount () {
        ...
        axios.get(Constants.ApiEndpoint + ApiRoutes.LinkStatuses + '/' + this.props.match.params.id, {
            headers: {
                'Content-Type': 'application/json',
            }
        })
...
/*<Codenesium>
    <Hash>389e8331cf4d17c0a3a263d509cecf2a</Hash>
</Codenesium>*/