...
export default class MachineSearchComponent extends React.Component<MachineSearchComponentProps, MachineSearchComponentState> {

    state = ({deleteSubmitted:false, deleteSuccess:false, deleteResponse:'', records:new Array<MachineViewModel>(), filteredRecords:new Array<MachineViewModel>(), searchValue:'', loading:false, loaded:true, errorOccurred:false, errorMessage:''});
    
    componentDidMount () {
        this.loadRecords();
    }

    handleEditClick(e:any, row:Api.MachineClientResponseModel) {
         this.props.history.push(ClientRoutes.Machines + '/edit/' + row.id);
    }

    handleDetailClick(e:any, row:Api.MachineClientResponseModel) {
    ...
    handleDeleteClick(e:any, row:Api.MachineClientResponseModel) {
        axios.delete(Constants.ApiEndpoint + ApiRoutes.Machines + '/' + row.id,
        {
            headers: {
   ...
   loadRecords(query:string = '') {
	   ...
	   let searchEndpoint = Constants.ApiEndpoint + ApiRoutes.Machines + '?limit=100';
	   ...
	   .then(resp => {
		   ...
		   console.log(error);
		   this.setState({records:new Array<MachineViewModel>(),filteredRecords:new Array<MachineViewModel>(), loading:false, loaded:false, errorOccurred:true, errorMessage:'Error from API'});
	   })
    }
        ...
        if(this.state.loading) {
            ...
            <ReactTable 
                ...
                columns={[{
                    ...
                    {
                        Header: 'Actions',
                        Cell: row => (<div><button className="btn btn-sm" onClick={e => {this.handleDetailClick(e, row.original as Api.MachineClientResponseModel)}} ><i className="fas fa-search"></i></button>
                        &nbsp;<button className="btn btn-primary btn-sm" onClick={e => {this.handleEditClick(e, row.original as Api.MachineClientResponseModel)}} ><i className="fas fa-edit"></i></button>
                        &nbsp;<button className="btn btn-danger btn-sm" onClick={e => {this.handleDeleteClick(e, row.original as Api.MachineClientResponseModel)}} ><i className="far fa-trash-alt"></i></button>
                        </div>)
                    }],
...
/*<Codenesium>
    <Hash>28ef905128740fbdca85e8a0dd62be39</Hash>
</Codenesium>*/